...
#
	...
	{
		cat <<-EOF >&2
			$PROGNAME: usage: $PROGNAME [--help] [--debug] [--verbose] [--noop] [--config-dir {path}] [--docker-host {spec}] [--docker-context {name}] [--database-user-name {name}] [--database-host-name {name}] [--database-host-port {name}]

			--database-user-name {name}  Specify the database host name (default: $DATABASE_USER_NAME)
			--database-host-name {name}  Specify the database host name (default: $DATABASE_HOST_NAME)
			--database-host-port {name}  Specify the database host port (default: $DATABASE_HOST_PORT)
...
# Load the default configuration files.
	...
	while [ $# -gt 0 ]; do
		...
		--database-host-name|--database-host-name=?*)
			case "$1" in
			--database-host-name)	DATABASE_HOST_NAME="$2"; shift;;
			--database-host-name=?*)	DATABASE_HOST_NAME="${1--database-host-name=}";;
			esac;;
		--database-host-port|--database-host-port=?*)
			case "$1" in
			--database-host-port)	DATABASE_HOST_PORT="$2"; shift;;
			--database-host-port=?*)	DATABASE_HOST_PORT="${1#--database-host-port=}";;
			esac;;
		-c)
			if [ -r "$2" ]; then
				source "$2";
...
# Initialise defaults
#
	if [ ! "$DATABASE_HOST_NAME" ]; then
		if [ "$PGHOST" ]; then
			DATABASE_HOST_NAME="$PGHOST";
		else
...
#		DATABASE_HOST_NAME="$POSTGRESQL_SERVICE_HOST";
#	fi;
...
#	PASSWORD=`execute kubectl get secret --namespace postgres postgresql-backup -o yaml | jq '.data'`;
#
#	if [ $? -ne 0 ]; then
...
#	fi;
	...
	execute echo "$DATABASE_HOST_NAME:$DATABASE_HOST_PORT:template1:$DATABASE_USER_NAME:$POSTGRES_PASSWORD" >>$HOME/.pgpass;
	execute echo "$DATABASE_HOST_NAME:$DATABASE_HOST_PORT:postgres:$DATABASE_USER_NAME:$POSTGRES_PASSWORD" >>$HOME/.pgpass;
	execute echo "$DATABASE_HOST_NAME:$DATABASE_HOST_PORT:repmgr:$DATABASE_USER_NAME:$POSTGRES_PASSWORD" >>$HOME/.pgpass;
...
#
	...
	then
		...
		set -o pipefail;
		#if ! execute pg_dumpall -g -h "$DATABASE_HOST_NAME" -U "$DATABASE_USER_NAME" | gzip >$FINAL_BACKUP_DIR"globals".sql.gz.in_progress; then
		PG_DUMPALL=$(execute pg_dumpall -g -h "$DATABASE_HOST_NAME" -U "$DATABASE_USER_NAME");

		if [ $? -ne 0 ]; then
			error "Failed to produce globals backup";
		else
			#mv $FINAL_BACKUP_DIR"globals".sql.gz.in_progress $FINAL_BACKUP_DIR"globals".sql.gz;
			echo "$PG_DUMPALL" | gzip >$FINAL_BACKUP_DIR"globals".sql.gz;
		fi;
		set +o pipefail;
	else
		echo "None";
...
#
	...
	SCHEMA_ONLY_DB_LIST=$(execute psql -h "$DATABASE_HOST_NAME" -U "$DATABASE_USER_NAME" -At -c "$SCHEMA_ONLY_QUERY" postgres);
	...
	for DATABASE in $SCHEMA_ONLY_DB_LIST; do
		...
		echo "$DATABASE_HOST_NAME:$DATABASE_HOST_PORT:$DATABASE:$DATABASE_USER_NAME:$POSTGRES_PASSWORD" >>$HOME/.pgpass;

		set -o pipefail;
		if ! execute pg_dump -Fp -s -h "$DATABASE_HOST_NAME" -U "$DATABASE_USER_NAME" "$DATABASE" | gzip >$FINAL_BACKUP_DIR"$DATABASE"_SCHEMA.sql.gz.in_progress; then
...
#
	...
	for DATABASE in $(execute psql -h "$DATABASE_HOST_NAME" -U "$DATABASE_USER_NAME" -At -c "$FULL_BACKUP_QUERY" postgres); do
		...
		then
			...
			set -o pipefail;
			if ! execute pg_dump -Fp -h "$DATABASE_HOST_NAME" -U "$DATABASE_USER_NAME" "$DATABASE" | gzip >$FINAL_BACKUP_DIR"$DATABASE".sql.gz.in_progress; then
		...
		then
			...
			if ! execute pg_dump -Fc -h "$DATABASE_HOST_NAME" -U "$DATABASE_USER_NAME" "$DATABASE" -f $FINAL_BACKUP_DIR"$DATABASE".custom.in_progress; then
